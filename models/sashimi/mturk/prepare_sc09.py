    ...
    for i, digit in enumerate(digits):
        try:
            ...
            raise FileExistsError(
                f"{target_dir}/{digit} already exists, please delete it before running this script."
            )
        for index in indices[i]:
            if not discard:
                shutil.copy(
                    f"{src_dir}/{src_files[index]}",
                    f"{target_dir}/{digit}/{src_files[index]}",
                )
            else:
                shutil.copy(
                    f"{src_dir}/{src_files[index]}",
                    f'{target_dir}/{digit}/{src_files[index].split("/")[-1]}',
                )
...
def standardize_filenames(target_dir):
    i = 0
    for digit in digits:
        if not os.path.exists(f"{target_dir}/{digit}/"):
            continue
...
def grab_indices(probs, samples_per_class=50):
    ...
    for digit in range(10):
        ...
        random_indices[digit] = np.random.choice(
            np.where(probs.argmax(1) == digit)[0], samples_per_class, replace=False
        )
...
def prepare(
    ...
    files = list(
        natsorted(
            [
                e
                for e in os.listdir(f"{sample_dir}/{n_samples}-{method}")
                if e.endswith(".wav")
            ]
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--methods", type=str, nargs="+", help="methods to prepare", required=True
    )
    ...
    parser.add_argument(
        "--target_dir", type=str, default="sc09/sc09-unconditional-exp-confident-repro/"
    )
