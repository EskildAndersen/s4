...
class MegaBlock(nn.Module):
    ...
    def __init__(
        ...
        **ssm_args,  # pass other keyword arguments to the SSM kernels
        ...
        self.attention_activation_fn = (
            ...
            else Activation(attention_activation)
        )
        self.scaling = self.d_attin**-0.5 if attention_activation == "softmax" else None

        # Configure dropout
        if drop_attin is None:
            drop_attin = dropout
        if drop_attout is None:
            drop_attout = dropout
        if drop_ffn is None:
            drop_ffn = dropout
        ...
        self.drop_attout = (
            dropout_fn(drop_attout) if drop_attout > 0.0 else nn.Identity()
        )
    ...
    def element_attention(self, q, k, padding_mask, attn_mask, before_attn_fn):
        ...
        if padding_mask is not None:
            ...
            lengths = lengths.clamp(min=1.0).unsqueeze(
                -1
            )  # (B K 1 1)  TODO finish transcribing
        else:
            lengths = slen
            inverse_mask = None
        ...
        **kwargs,
            ...
            if l_ctx < self.l_chunk:
                ...
                if padding_mask is not None:
                    padding_mask = rearrange(
                        padding_mask, "b (k c) -> b k c", c=self.l_chunk
                    )
        ...
        if self.attention_activation_fn is None:  # Softmax case
            attn_weights = self.softmax_attention(
                q, k, padding_mask, attn_mask, before_attn_fn
            )
        else:
    ...
    def extra_repr(self) -> str:
        return "d_model={}, d_attin={}, d_attout={}, d_state={}, chunk={}, attn_act={}, prenorm={}".format(
            self.d_model,
            self.d_attin,
            self.d_attout,
            self.d_state,
...
class MultiHeadEMA(nn.Module):
    ...
    def forward(
        ...
        if state is not None:
            ...
            if self.bidirectional:
                ...
                k = F.pad(k1, (l_kernel - 1, 0)) + F.pad(
                    k2.flip(-1), (0, l_kernel - 1)
                )  # (H 2*L-1)
    ...
    def extra_repr(self) -> str:
        return "edim={}, N={}, bidirectional={}, trunction={}".format(
            self.H, self.N, self.bidirectional, self.l_max
        )
    ...
    def rotary(self, x):
        ...
        if self.sine is None or n > self.sine.size(0):
            self.sine, self.cosine = (
                RotaryRelativePositionalBias.get_sinusoid_embeddings(n, d)
            )
