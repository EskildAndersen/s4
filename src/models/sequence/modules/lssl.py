...
class Platypus(SequenceModule):
    ...
    def __init__(
        ...
        if self.learn == 0:
            if measure == "identity":  # for testing
                ...
                A = (
                    torch.randn(self.N, self.N) / self.N
                )  # E[AA^T] = (1/N)I -- empirically I nans out
                B = torch.ones(
                    self.N
                )  # based on HiPPO matrices; worth trying random, haven't tried
                self.transition = transition.ManualAdaptiveTransition(self.N, A, B)
            elif measure == "legt":
                # self.transition = transition.LegTAdaptiveTransition(self.N)
                self.transition = transition.LegTTriDInverseAdaptiveTransition(
                    self.N, **measure_args
                )
            elif measure == "cheb":
                self.transition = transition.ChebITriDInverseAdaptiveTransition(
                    self.N, **measure_args
                )
            elif measure == "chebii":
                self.transition = transition.ChebIITriDInverseAdaptiveTransition(
                    self.N, **measure_args
                )
            elif measure == "lagt":
                self.transition = transition.LagTCumsumAdaptiveTransition(
                    self.N, **measure_args
                )
            elif measure == "glagt":
                self.transition = transition.GLagTToeplitzAdaptiveTransition(
                    self.N, **measure_args
                )
            elif measure == "legs":
                self.transition = transition.LegSTriDInverseAdaptiveTransition(
                    self.N, **measure_args
                )
            elif measure == "jac":
                self.transition = transition.JacTriDInverseAdaptiveTransition(
                    self.N, **measure_args
                )
            else:
            ...
            if measure == "random":
                A = (
                    torch.randn(self.N, self.N) / self.N
                )  # E[AA^T] = (1/N)I . empirically I doesn't work, dunno why
                B = torch.ones(
                    self.N
                )  # based on HiPPO matrices; worth trying random, haven't tried
                self.transition = transition.ManualAdaptiveTransition(
                    self.N, A, B, **kwargs
                )
            elif measure == "legt":
                self.transition = transition.LegTTriDInverseAdaptiveTransition(
                ...
                self.transition = transition.LagTTriDInverseAdaptiveTransition(
                    self.N, **kwargs
                )
            elif measure == "legs":
                self.transition = transition.LegSTriDInverseAdaptiveTransition(
                ...
                self.transition = transition.LagTToeplitzAdaptiveTransition(
                    self.N, **kwargs
                )
            else:
                raise NotImplementedError
            ...
            B = B[:, 0]
            self.transition = transition.ManualAdaptiveTransition(
                self.N, A, B, trainable=True, lr=lr
            )
        else:
            raise NotImplementedError
        ...
        if self.dt.init == "uniform":
            log_dt = torch.linspace(
                math.log(self.dt.min), math.log(self.dt.max), self.d
            )
        elif self.dt.init == "random":
            log_dt = torch.rand(self.d) * (
                math.log(self.dt.max) - math.log(self.dt.min)
            ) + math.log(self.dt.min)
        else:
            raise NotImplementedError
        ...
        if self.dt.learn:
            ...
            self.log_dt._lr = (
                self.dt.lr
            )  # register the parameter for the optimizer to reduce lr
        else:
    ...
    def forward(self, u, *args, state=None, **kwargs):
        ...
        dt = torch.exp(
            self.log_dt
        )  # Note: if dt is not learnable this slightly wastes computation, but it isn't a bottleneck
        ...
        _learn = (
            (self.dt.learn or self.learn) and self.training
        )  # need to learn and it's training time # TODO this ignores the last training minibatch if no test time shift (prev batch's K gets cached)... should recalculate A in the last_len check ideally
        ...
        if _conv:
            ...
            kb.append(B)
        if _noise:
            noise = self.noise * torch.randn(
                self.d, self.N, dtype=u.dtype, device=u.device
            )  # (H, N)
            kb.append(noise)
        ...
        if len(kb) > 0:
            ...
            k_noise, k_conv = torch.split(
                krylovs, split_size_or_sections=[int(_noise), int(_conv)], dim=0
            )
        ...
        # Convolution
        y = linear_system_from_krylov(
            u, self.C, self.D, self.k[..., : u.shape[0]]
        )  # (L, B, H, M)
        if _noise:
        ...
        if state is not None:  # (B, H, N)
            ...
            ATC, ATL = krylov(
                u.shape[0],
                A.transpose(-1, -2),
                self.C.transpose(0, 1),
                return_power=True,
            )  # (M, H, N, L), (H, N, N) represents A^T C and (A^T)^L
            ...
            with torch.no_grad():
                ...
                next_state = next_state + contract(
                    "lbh, hnl -> bhn", u.flip(0), self.k[:..., u.shape[0]]
                )  # (B, H, N)
