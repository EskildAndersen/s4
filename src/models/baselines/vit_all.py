...
class VisionTransformer(SequenceModule):
    ...
    def __init__(
        ...
        self.num_features = self.d_model = (
            d_model  # num_features for consistency with other models
        )
        ...
        if use_cls_token:
            ...
            self.dist_token = (
                nn.Parameter(torch.zeros(1, 1, d_model)) if distilled else None
            )
        else:
        ...
        if use_pos_embed:
            self.pos_embed = nn.Parameter(
                torch.zeros(1, num_patches + self.num_tokens, d_model)
            )
    ...
    def forward_features(self, x):
        ...
        if self.track_norms:
            output_norms = [torch.mean(x.detach() ** 2)]

        for block in self.blocks:
            x, _ = block(x)
            if self.track_norms:
                output_norms.append(torch.mean(x.detach() ** 2))
        x = self.norm(x)
    ...
    if isinstance(m, (nn.Linear)):
        if n.startswith("head"):
            ...
            if jax_impl:
                ...
                if m.bias is not None:
                    nn.init.zeros_(m.bias)
                dense_init_fn_ = partial(trunc_normal_, std=0.02)
                if isinstance(m, nn.Linear):
