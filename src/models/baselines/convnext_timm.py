...
class Stem(nn.Module):
    def __init__(
        ...
        if stem_type == "patch":
            ...
            assert (
                stem_l_max is not None
            ), "need to provide a stem_l_max to use stem=new_s4nd_patch"
            stem_layer_copy["l_max"] = stem_l_max

            self.pre_stem = nn.Identity()
            self.stem = utils.instantiate(
                registry.layer, stem_layer_copy, in_ch, out_channels=stem_channels
            )
            ...
            self.pre_stem = nn.Conv2d(
                in_ch, stem_channels, kernel_size=1, stride=1, padding=0
            )
            # s4 + norm + avg pool + linear
            self.stem = S4DownSample(
                stem_layer_copy,
                stem_channels,
                out_ch,
                stride=patch_size,
    ...
    def __init__(
        ...
        if in_chs != out_chs or stride > 1:
            ...
            if downsample_type == "s4nd":
                ...
                downsample_layer_copy["l_max"] = (
                    img_size  # always need to update curr l_max
                )
            ...
            # strided conv
                ...
                self.norm_layer = norm_layer(in_chs)
                self.downsample = Conv2dWrapper(
                    in_chs, out_chs, kernel_size=stride, stride=stride
                )
        # else:
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        for i in range(4):
            ...
            stride = (
                1 if i == 0 and stem_stride == 4 else 2
            )  # stride 1 is no downsample (because already ds in stem)
            ...
            prev_chs = out_chs
            curr_img_size = [
                x // stride for x in curr_img_size
...
class Conv3d(nn.Conv3d):
    def __init__(
        self, in_ch, out_ch, kernel_size, stride, padding=0, groups=1, factor=False
    ):
        super().__init__(
            in_ch, out_ch, kernel_size, stride=stride, padding=padding, groups=groups
        )
        self.factor = factor
        self.in_ch = in_ch
        self.out_ch = out_ch
        ...
        if self.factor:
            self.weight = nn.Parameter(
                self.weight[:, :, 0, :, :]
            )  # Subsample time dimension
            self.time_weight = nn.Parameter(
                self.weight.new_ones(self.kernel_size[0]) / self.kernel_size[0]
            )
        else:
            pass
...
class ConvNeXtBlock3D(nn.Module):
    ...
    def __init__(
        ...
        self.norm = norm_layer(dim)
        self.mlp = mlp_layer(
            dim, int(mlp_ratio * dim), act_layer=nn.GELU, drop=drop_mlp
        )
        self.gamma = (
            nn.Parameter(ls_init_value * torch.ones(dim)) if ls_init_value > 0 else None
        )
    ...
    def __init__(
        ...
        if in_chs != out_chs or np.any(np.array(stride) > 1):
            ...
            if downsample_type == "s4nd":
                ...
                downsample_layer_copy = copy.deepcopy(stage_layer)
                downsample_layer_copy["l_max"] = (
                    video_size  # always need to update curr l_max
                )
                self.norm_layer = norm_layer(in_chs)
                # mimics strided conv but w/s4
                self.downsample = S4DownSample(
            ...
            # strided conv
                ...
                print("strided conv downsample")
                self.norm_layer = norm_layer(in_chs)
                self.downsample = Conv3dWrapper(
                    in_chs, out_chs, kernel_size=stride, stride=stride, factor=factor_3d
                )
...
class Stem3d(nn.Module):
    def __init__(
        ...
        if stem_type == "patch":
            ...
            stem_layer_copy["l_max"] = stem_l_max
            s4_ds = utils.instantiate(
                registry.layer, stem_layer_copy, in_chans, out_channels=stem_channels
            )
            kernel_3d = [tempor_patch_size, spatial_patch_size, spatial_patch_size]
            self.stem = nn.Sequential(
                s4_ds,
    ...
    def __init__(
        ...
        **kwargs,  # catch all
        ...
        for i in range(4):
            ...
            tempor_stride = (
                temporal_stage_strides[i] if temporal_stage_strides is not None else 2
            )
            stride = (
                [1, 1, 1]
                if i == 0 and np.any(np.array(stem_stride) >= 2)
                else [tempor_stride, 2, 2]
            )  # stride 1 is no downsample (because already ds in stem)
...
def convnext_timm_tiny_2d_to_3d(model, state_dict, ignore_head=True, normalize=True):
    ...
    prefix = list(state_dict.keys())[0].split(".")[
        0
    ]  # grab prefix in the keys for state_dict params
    ...
    for key in sorted(model_scratch_params_dict.keys()):
        ...
        if "time_weight" in key:
            ...
            used_params = repeat(
                temporal_kernel_factor * loaded_params,
                "... h w -> ... t h w",
                t=temporal_dim,
            )
...
def convnext_timm_tiny_s4nd_2d_to_3d(model, state_dict, ignore_head=True, jank=False):
    ...
    for key in sorted(model_scratch_params_dict.keys()):
        ...
        if ".kernel.0" in key:
            ...
            new_state_dict[key_with_prefix] = state_dict[
                old_key_with_prefix.replace(".kernel.1", ".kernel.0")
            ]
        elif ".kernel.2" in key:
            print("FOUND .kernel.2, putting kernel 1 into kernel 2", key)
            new_state_dict[key_with_prefix] = state_dict[
                old_key_with_prefix.replace(".kernel.2", ".kernel.1")
            ]
        elif ignore_head and "head" in key:
            # ignore head weights
            print("found head key / parameter, ignore", key)
        ...
        # keys match
            ...
            try:
                if (
                    model_scratch_params_dict[key].ndim
                    != state_dict[old_key_with_prefix].ndim
                ):
                    print(
                        "matching keys, but shapes mismatched!  Need to inflate!", key
                    )
                    # need the index [-3], 3rd from last, the temporal dim
                    index = -3
                    ...
                    new_state_dict[key_with_prefix] = repeat(
                        state_dict[old_key_with_prefix],
                        "... h w -> ... t h w",
                        t=dim_len,
                    )  # torch.cat(dim_len * [loaded_params], axis=index)  # stack at this dim
                else:
