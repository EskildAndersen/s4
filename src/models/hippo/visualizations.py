...
def animate_hippo(
    ...
    plot_s4=False,
    s4_offset=6.0,
    plot_hippo_type="line",
    plot_measure_type="line",
    plot_coeff_type="line",
    size=1.0,
    plot_legend=True,
    plot_xticks=True,
    plot_box=True,
    ...
    if plot_measure:
        ...
        ln_measure = plt_lines(
            vals,
            np.zeros(len(vals)) + measure_offset,
            size=size,
            color="green",
            **label_args,
        )
    ...
    if len(plot_coeff) > 0:
        ln_coeffs = [plt_lines([], [], size=size, color="blue") for _ in plot_coeff]
        plt_lines(
