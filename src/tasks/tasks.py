        ...
        for name in self.torchmetric_names:
            if name.startswith("Accuracy"):
                if len(x.shape) > 2:
                    ...
                    self._tracked_torchmetrics[prefix][name].update(
                        x.transpose(1, 2), y.squeeze()
                    )
                    continue
...
class ForecastingTask(BaseTask):
    ...
    def __init__(self, norm="mean", **kwargs):
        ...
        if norm == "revnorm":
            self.encoder = ReversibleInstanceNorm1dInput(
                self.dataset.d_input, transposed=False
            )
            self.decoder = ReversibleInstanceNorm1dOutput(self.encoder)
        elif norm == "mean":
            self.encoder = TSNormalization(
                method="mean", horizon=self.dataset.dataset_train.forecast_horizon
            )
            ...
            self.encoder = TSNormalization(
                method="last", horizon=self.dataset.dataset_train.forecast_horizon
            )
            self.decoder = TSInverseNormalization(
        ...
        try:
            if hasattr(self.dataset.dataset_train, "mean"):
                ...
                self.mean = torch.tensor(
                    self.dataset.dataset_train.standardization["means"]
                )
                self.std = torch.tensor(
                    self.dataset.dataset_train.standardization["stds"]
                )
            else:
                self.mean = None
                self.std = None
    ...
    def metrics(
        self, x, y, state=None, timestamps=None, ids=None
    ):  # Explicit about which arguments the decoder might pass through, but can future-proof with **kwargs
...
class VideoTask(BaseTask):
    ...
    def get_torchmetrics(self, prefix):
        vid_to_average_logits = {
            vid: torch.mean(torch.stack(logits, dim=0), dim=0)
            for vid, logits in self._vid_to_logits[prefix].items()
        }
        # y is (label, vid) pair
        all_labels = torch.stack(
            list(self._vid_to_label[prefix].values()), dim=0
        )  # (n_videos)
        all_logits = torch.stack(
            list(vid_to_average_logits.values()), dim=0
        )  # (n_videos, n_classes)
...
class ImageNetTask(BaseTask):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(
            ...
            loss=kwargs.get(
                "loss", None
            ),  # we still create the base loss here, but will overide below
