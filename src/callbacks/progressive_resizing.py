...
class ProgressiveResizing(Callback):
    def __init__(self, stage_params: list):
        ...
        assert all(
            [{"resolution", "epochs"} <= set(stage.keys()) for stage in stage_params]
    ...
    def on_train_start(self, trainer, model) -> None:
        ...
        if trainer.current_epoch == 0:
            ...
            print(
                "Progressive Resizing: Restarting at Stage {}".format(
                    self._current_stage
                )
            )
    ...
    def _update_dataloaders(self, trainer, model):
        ...
        model.hparams.loader.train_resolution = self.stage_params[self._current_stage][
            "resolution"
        ]
        trainer.reset_train_dataloader(model)

        print(
            "\tChanged resolution to {}".format(
                self.stage_params[self._current_stage]["resolution"]
            )
        )
    ...
    def _update_model(self, trainer, model):
        ...
        print(
            "\tChanged bandlimit to {}".format(
                self.stage_params[self._current_stage]["bandlimit"]
            )
        )
