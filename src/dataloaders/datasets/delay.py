...
class DelayTrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.FloatTensor(
            whitesignal(self.L * self.dt, self.dt, self.freq)
        )  # (l_seq)
        y = torch.stack(
            [
...
class DelayEvalDataset(torch.utils.data.TensorDataset):
    def __init__(self, samples, l_seq=1024, n_lag=1, l_lag=None, dt=1e-3, freq=1.0):
        ...
        X = torch.FloatTensor(
            whitesignal(
                self.L * self.dt, self.dt, self.freq, batch_shape=(self.samples,)
            )
        )  # (samples, l_seq, 1)
        Y = torch.stack(
            [
                F.pad(
                    X[:, : self.L - i * self.l_lag], (i * self.l_lag, 0)
                )  # manually subtract from self.L otherwise error in i=0 case
                for i in range(1, self.n_lag + 1)
            ],
            dim=-1,
        )  # (batch, l_seq, n_lag)
